...
def use_NARX_model_in_simulation(ulist, ylist, model, na, nb):
    # free-run like your original (warm-up 50)
    ylist = list(ylist[:50])
    ...
    with torch.no_grad():
        for unow in ulist[50:]:
            x = torch.tensor(upast + ypast, dtype=torch.float32, device=device).unsqueeze(0)
            out = model(x).squeeze().float().cpu().item()
            upast.append(float(unow)); upast.pop(0)
            ypast.append(out); ypast.pop(0)
            ylist.append(out)
...
def narx_hidden_prediction_rmse(model, na, nb, pred_npz):
    ...
    hidden-test-prediction-submission-file.npz must contain:
      - upast: (N, 15)
      - thpast: (N, 15)
      - ground truth: one of ['thnow', 'thnow_true', 'y', 'y_true', 'target']  shape (N, 1) or (N,)
    """
    if pred_npz is None:
        return None
    upast = pred_npz['upast']
    thpast = pred_npz['thpast']
    gt_key = get_first_key(pred_npz, ['thnow_true', 'thnow', 'y_true', 'y', 'target'])
    if gt_key is None:
        return None
    y_true = pred_npz[gt_key].reshape(-1)

    if upast.shape[1] < nb or thpast.shape[1] < na:
        return None  # not enough lags for this (na, nb)

    u_slice = upast[:, -nb:] if nb > 0 else np.zeros((upast.shape[0], 0), dtype=np.float32)
    y_slice = thpast[:, -na:] if na > 0 else np.zeros((thpast.shape[0], 0), dtype=np.float32)
    X_hidden = np.concatenate([u_slice, y_slice], axis=1).astype(np.float32)

    model.eval()
    with torch.no_grad():
        preds = model(torch.tensor(X_hidden, dtype=torch.float32, device=device)).squeeze().float().cpu().numpy()
    return compute_rmse(preds, y_true)

def narx_hidden_simulation_rmse(model, na, nb, sim_npz):
    """
    hidden-test-simulation-submission-file.npz must contain:
      - u: (T,)
      - th: (T,) ground-truth full trajectory
    """
    if sim_npz is None or ('u' not in sim_npz) or ('th' not in sim_npz):
        return None
    u_seq = sim_npz['u'].astype(np.float32)
    th_true = sim_npz['th'].astype(np.float32)
    if len(u_seq) < 60 or len(th_true) < 60:
        return None

    th_sim = use_NARX_model_in_simulation(list(u_seq), list(th_true), model, na, nb).cpu().numpy()
    if len(th_sim) != len(th_true):
        L = min(len(th_sim), len(th_true))
        th_sim = th_sim[:L]; th_true = th_true[:L]
    return compute_rmse(th_sim, th_true)
            ...
            for hl in [1, 2]:
                for hn in [32, 64]:
                    ...
                    with torch.no_grad():
                        ...
                        rmse_val_pred = compute_rmse(preds_val, y_val.numpy())

                    # Hidden prediction RMSE (one-step)
                    rmse_hidden_pred = narx_hidden_prediction_rmse(model_narx, na, nb, hidden_pred_npz)

                    # Hidden simulation RMSE (free-run)
                    ...
                    print(f"NARX na:{na} nb:{nb} hl:{hl} hn:{hn} act:{act_name} "
                          f"→ ValPredRMSE: {rmse_val_pred:.5f} "
                          f"| HiddenPredRMSE: {('NA' if rmse_hidden_pred is None else f'{rmse_hidden_pred:.5f}')} "
                          f"| HiddenSimRMSE: {('NA' if rmse_hidden_sim is None else f'{rmse_hidden_sim:.5f}')}")

                    # Track best by hidden simulation if available, else by val pred
                    score = rmse_hidden_sim if rmse_hidden_sim is not None else rmse_val_pred
...
if best_model_narx is not None:
    ...
    print(f"\nBest NARX config: na:{best_config_narx[0]} nb:{best_config_narx[1]} hl:{best_config_narx[2]} "
          f"hn:{best_config_narx[3]} act:{best_config_narx[4]} | Score (prefers HiddenSim): {best_rmse_narx:.5f}")
...
if best_model_narx is not None and hidden_sim_npz is not None:
    ...
    model.eval()
    u_test = list(hidden_sim_npz['u'])
    th_test = list(hidden_sim_npz['th'])
...
def lstm_hidden_prediction_rmse(model_lstm):
    ...
    return compute_rmse(pred_theta_unnorm, th_now_true)

def lstm_hidden_sim_rmse(model_lstm, seq_len):
    """Teacher-forced next-step over hidden simulation set."""
    if hidden_sim_npz is None or ('u' not in hidden_sim_npz) or ('th' not in hidden_sim_npz):
        return None
    u_h = hidden_sim_npz['u'].astype(np.float32)
    th_h = hidden_sim_npz['th'].astype(np.float32)
    if len(th_h) <= seq_len:
        return None
    ...
    return compute_rmse(pred_theta, gt_theta)
    ...
    for num_layers in [1, 2]:
        for bidirectional in [False, True]:
            ...
            print(f"LSTM hs:{hidden_size} nl:{num_layers} bi:{int(bidirectional)} "
                  ...
                  f"| HiddenPredRMSE: {('NA' if rmse_hidden_pred_lstm is None else f'{rmse_hidden_pred_lstm:.5f}')} "
                  f"| HiddenSimRMSE: {('NA' if rmse_hidden_sim_lstm is None else f'{rmse_hidden_sim_lstm:.5f}')}")
...
if input_data is not None and ('u' in input_data) and ('th' in input_data):
    u_hidden = input_data["u"].astype(np.float32)
    th_hidden = input_data["th"].astype(np.float32)
    ...
    print("⚠ Hidden simulation file missing or lacks keys 'u'/'th' — skipped LSTM export.")